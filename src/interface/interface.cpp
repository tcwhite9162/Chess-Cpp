...
static const std::unordered_map<std::string, CommandHandler> commandTable = {
    {"position", [](const std::string& line, Board& board) { handlePosition(line, board); }},

    {"move", [](const std::string& line, Board& board) { handleMove(line, board); }},
    {"moves", [](const std::string& line, Board& board) { handleMoves(line, board); }},

    {"perft", [](const std::string& line, Board& board) { handlePerft(line, board, PerftMode::Single); }},
    {"perft-divide", [](const std::string& line, Board& board) { handlePerft(line, board, PerftMode::Divide); }},
    {"perft-suite", [](const std::string& line, Board& board) { handlePerft(line, board, PerftMode::Suite); }},
    {"perft-all", [](const std::string& line, Board& board) { handlePerft(line, board, PerftMode::All); }},

    {"d", [](std::string&, const Board& board) { showState(board, true); }},
    {"p", [](std::string&, const Board& board) { showState(board, false); }},

    {"h", [](std::string&, Board&) { showCommands(); }},
    {"help", [](std::string&, Board&) { showCommands(); }},

    {"clear", [](std::string&, Board&) { clearScreen(); }},
    {"c", [](std::string&, Board&) { clearScreen(); }},

    {"search", [](const std::string& line, Board& board) { handleSearch(line, board, false); }},
    {"search-range", [](const std::string& line, Board& board) { handleSearch(line, board, true); }}};
...
void handlePerft(const std::string& line, Board& board, PerftMode mode) {
    ...
    if (!isDigitsOnly(d)) {
        std::cout << "error: depth must be an integer\n";
        return;
    }
...
void showCommands() {
    ...
    std::cout << "'p' - print board position\n";
    std::cout << "'d' - print board position with debug info\n\n";

    std::cout << "'search <depth>'       - search for best move at <depth>\n";
    std::cout << "'search-range <depth>' - search for best move at all depths [1 ... <depth>]\n\n";
...
std::optional<Move> parseMoveStr(const std::string& moveStr, const Board& board) {
    ...
    if (moveStr.length() == 5) {
        switch (moveStr[4]) {
        case 'q':
            flags |= (Flags::FLAG_PROMO_Q | Flags::FLAG_PROMOTION);
            break;
        case 'r':
            flags |= (Flags::FLAG_PROMO_R | Flags::FLAG_PROMOTION);
            break;
        case 'b':
            flags |= (Flags::FLAG_PROMO_B | Flags::FLAG_PROMOTION);
            break;
        case 'n':
            flags |= (Flags::FLAG_PROMO_N | Flags::FLAG_PROMOTION);
            break;
        default:
    ...
    if (board.getPiece(from) == currKing) {
        if (from == Constants::Castling::WHITE_KING_START && to == Constants::Castling::WHITE_KINGSIDE_END)
            flags |= Flags::FLAG_CASTLE_WK;
        if (from == Constants::Castling::WHITE_KING_START && to == Constants::Castling::WHITE_QUEENSIDE_END)
            flags |= Flags::FLAG_CASTLE_WQ;
        if (from == Constants::Castling::BLACK_KING_START && to == Constants::Castling::BLACK_KINGSIDE_END)
            flags |= Flags::FLAG_CASTLE_BK;
        if (from == Constants::Castling::BLACK_KING_START && to == Constants::Castling::BLACK_QUEENSIDE_END)
            flags |= Flags::FLAG_CASTLE_BQ;
    }
