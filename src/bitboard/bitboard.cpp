...
u64 kingAttacks[64];
u64 pawnAttacksWhite[64];
u64 pawnAttacksBlack[64];
...
void initPawnAttacks() {
    for (int sq = 0; sq < 64; sq++) {
        ...
        if (r < 7 && f > 0)
            white |= 1ULL << (sq + 7); // down-left
        if (r < 7 && f < 7)
            white |= 1ULL << (sq + 9); // down-right

        if (r > 0 && f < 7)
            black |= 1ULL << (sq - 7); // up-right
        if (r > 0 && f > 0)
            black |= 1ULL << (sq - 9); // up-left

        pawnAttacksWhite[sq] = white;
        pawnAttacksBlack[sq] = black;
    }
}
